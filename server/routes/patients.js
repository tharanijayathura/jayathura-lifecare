...
// 7b. getActiveCart() - Get the patient's current draft order (cart)
router.get('/cart/active', authMiddleware, async (req, res) => {
  try {
    if (req.user.role !== 'patient' && req.user.role !== 'admin' && req.user.role !== 'pharmacist') {
      return res.status(403).json({ message: 'Access denied' });
    }
    ...
    const order = await Order.findOne({ 
      ...
      .sort({ createdAt: -1 })
      .populate('items.medicineId', 'name price stock image');
...
router.post('/cart/add', authMiddleware, async (req, res) => {
  try {
    ...
    if (orderId) {
      ...
      // Enforce status checks - cannot add to already finalized/confirmed orders
      if (order.status !== 'draft' && order.status !== 'pending') {
        return res.status(400).json({ message: 'Cannot modify a confirmed or completed order' });
      }
      ...
      // Try to find an existing draft order first, to avoid creating duplicates
      order = await Order.findOne({ patientId: req.user._id, status: 'draft' })
        .sort({ createdAt: -1 });
