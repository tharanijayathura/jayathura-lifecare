...
const TopSellingTable = ({ items, formatCurrency }) => {
  ...
  return (
    <Card>
      <CardContent>
        <Typography variant="h6" gutterBottom sx={{ fontWeight: 600, mb: 2 }}>Top Selling Medicines</Typography>
        <TableContainer>
          <Table size="small">
            <TableHead>
              <TableRow>
            ...
            <TableBody>
              {items.map((item, index) => (
                <TableRow key={index} hover>
                  <TableCell>
                    <Chip label={`#${index + 1}`} size="small" color={index === 0 ? 'primary' : index === 1 ? 'success' : 'default'} />
                  </TableCell>
