...
const CategoryBreakdown = ({ breakdown, formatCurrency }) => {
  if (!breakdown || breakdown.length === 0) return null;
  const total = breakdown.reduce((sum, c) => sum + (c.sales || 0), 0);
  return (
    <Card>
      <CardContent>
        <Typography variant="h6" gutterBottom sx={{ fontWeight: 600 }}>Category Breakdown</Typography>
        <Box sx={{ mt: 2 }}>
            ...
            return (
              <Box key={index} sx={{ mb: 2 }}>
                ...
                <LinearProgress
                  variant="determinate"
                  value={percentage}
                  sx={{ height: 8, borderRadius: 4, bgcolor: 'grey.200', '& .MuiLinearProgress-bar': { bgcolor: index === 0 ? 'primary.main' : index === 1 ? 'success.main' : 'info.main' } }}
                />
